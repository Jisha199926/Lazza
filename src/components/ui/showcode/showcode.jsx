...
const Showcode = ({ title, code, children, customCardClass, customCardHeaderClass, customCardBodyClass, customCardFooterClass }) => {
  ...
  return (
    <div className={`box ${customCardClass}`}>
      ...
      <div ref={randomValueRef} className={`box-body ${customCardBodyClass}`} style={{ display: showCode ? 'none' : '' }}>
        {React.Children.map(children, child => React.cloneElement(child))}
      </div>
      <div className={`box-footer border-top-0 ${customCardFooterClass}`} style={{ display: showCode ? 'block' : 'none' }}>
